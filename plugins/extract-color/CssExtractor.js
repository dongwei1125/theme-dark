/**
 * reference: https://github.com/hzsrc/webpack-theme-color-replacer/blob/master/src/CssExtractor.js
 */

// \n和备注
var Reg_Lf_Rem = /\\\\?n|\n|\\\\?t|\\\\?r|\/\*[\s\S]+?\*\//g
// 保留注释 /* extract-color ignore */
var IgnoreReg = /^\/\* extract-color ignore \*\/$/
...
module.exports = function extractCss(src, options) {
  ...
  while (true) {
    ...
    if (cssEnd > -1 && cssEnd > nameEnd && nameEnd > nameStart) {
      ...
      if (cssCode.replace(/\$\{/g, '').indexOf('{') > -1) {
        // @keyframes
...
function ignoreComments(cssCode) {
  function replacement(match) {
    return IgnoreReg.test(match) ? match : ''
  }

  return cssCode.replace(Reg_Lf_Rem, replacement)
}
